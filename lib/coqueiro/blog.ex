defmodule Coqueiro.Blog do
  ...
  def subscribe_posts(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(Coqueiro.PubSub, "session:#{key}:posts")
  end

  defp broadcast(%Scope{} = scope, message) do
    key = scope.id

    Phoenix.PubSub.broadcast(Coqueiro.PubSub, "session:#{key}:posts", message)
  end
  ...
  def get_post!(%Scope{} = scope, id) do
    Repo.get_by!(Post, id: id, session_id: scope.id)
  end
