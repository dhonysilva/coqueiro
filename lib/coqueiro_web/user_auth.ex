defmodule CoqueiroWeb.UserAuth do
  use CoqueiroWeb, :verified_routes
  ...
  @remember_me_cookie "_coqueiro_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      CoqueiroWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      CoqueiroWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule CoqueiroWeb.PageLive do
        use CoqueiroWeb, :live_view

        on_mount {CoqueiroWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{CoqueiroWeb.UserAuth, :require_authenticated}] do
    ...
    case Coqueiro.Accounts.get_organization_by_slug!(current_scope, slug) do
      %Coqueiro.Accounts.Organization{} = org ->
        if membership = Coqueiro.Accounts.get_membership(current_scope.user, org) do
          new_scope =
            current_scope
            |> Coqueiro.Accounts.Scope.put_organization(org)
            |> Coqueiro.Accounts.Scope.put_membership(membership)
