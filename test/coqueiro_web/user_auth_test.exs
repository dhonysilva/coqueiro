defmodule CoqueiroWeb.UserAuthTest do
  use CoqueiroWeb.ConnCase

  alias Phoenix.LiveView
  alias Coqueiro.Accounts
  alias Coqueiro.Accounts.Scope
  alias CoqueiroWeb.UserAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CoqueiroWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CoqueiroWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CoqueiroWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
